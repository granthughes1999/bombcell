...
from grant_config import get_probe_mode_overrides, load_config

BC_ROI_LABEL_COLUMN = "bc_ROI"
...
def save_phy_summary_labels(
    ...
    main_reason = (
        qm_df["main_reason"].astype(str).to_numpy()
        if "main_reason" in qm_df.columns
        else np.array(["" for _ in range(len(cluster_ids))], dtype=str)
    )
    export_df = pd.DataFrame(
        {
            "cluster_id": cluster_ids.astype(int),
            "bc_unitType": pd.Series(unit_type_string).astype(str).to_numpy(),
            "bc_classificationReason": main_reason,
            BC_ROI_LABEL_COLUMN: (
                roi_labels.astype(str) if roi_labels is not None else np.array(["" for _ in range(len(cluster_ids))], dtype=str)
            ),
        }
    )
    export_df.to_csv(ks_dir / "cluster_bc_classificationReason.tsv", sep="\t", index=False)
...
def export_results(results: Dict[str, Dict[str, Any]], output_root: Path, probes: Iterable[str], overwrite: bool) -> None:
    ...
    for probe in probes:
        ...
        if "roi_label" in entry and entry["roi_label"] is not None and len(entry["roi_label"]) == len(qm):
...
def numbered_run_root(recording_root: Path) -> Path:
    for idx in range(1, 1000):
        run_root = recording_root / f"bombcell_{idx:02d}"
        if not run_root.exists():
            run_root.mkdir(parents=True, exist_ok=False)
            return run_root
    raise RuntimeError(f"Could not allocate run root under {recording_root} after 999 attempts")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Simplified Bombcell runner for Grant workflows")
    parser.add_argument("--config", type=str, default=None, help="Path to recording_config.json")
    parser.add_argument("--mode", choices=["batch", "single_probe"], default="batch", help="Run mode")
...
def main() -> None:
    ...
    run_root = numbered_run_root(cfg["recording_root"])
    staged_dirs = stage_kilosort4(cfg["probe_kilosort_dirs"], run_root, probes, overwrite=args.overwrite)
    ...
    for probe in probes:
        ...
        try:
            ...
            if "unique_templates" in param_out:
                ...
                raise KeyError(
                    f"Could not determine cluster IDs for Phy export for probe {probe}. "
            ...
            save_phy_summary_labels(
                ks_dir=ks_dir,
                cluster_ids=cluster_ids,
                unit_type_string=np.asarray(unit_type_string),
    ...
    export_results(results, run_root, probes, overwrite=args.overwrite)
    print(f"\nExport complete: {run_root}")
