...
def label_units_by_tip_distance(
    ...
    _, _, _, _, _, _, channel_positions = bc.load_ephys_data(str(ks_dir))
    shank_y = channel_positions[:, 1].astype(float)
    if tip_position == "min_y":
        tip_y = float(np.nanmin(shank_y))
        dist_um = channel_positions[qm_df["maxChannels"].astype(int).to_numpy(), 1].astype(float) - tip_y
    elif tip_position == "max_y":
        tip_y = float(np.nanmax(shank_y))
        dist_um = tip_y - channel_positions[qm_df["maxChannels"].astype(int).to_numpy(), 1].astype(float)
    else:
