...
def load_env(env_path: str | Path | None = None) -> dict[str, str]:
    ...
    data = {
        "MOUSE": _require_env("MOUSE"),
        "BEHAVIORAL_FOLDER": _require_env("BEHAVIORAL_FOLDER"),
        'PROBE_A_CH_CONFIG': _require_env("PROBE_A_CH_CONFIG"),
        'PROBE_C_CH_CONFIG': _require_env("PROBE_C_CH_CONFIG"),
        'PROBE_D_CH_CONFIG': _require_env("PROBE_D_CH_CONFIG"),
        "NP_FILE": _require_env("NP_FILE"),
        "NWB_FILE": _require_env("NWB_FILE"),
        "DATE": _require_env("DATE"),
        "SESSION": _require_env("SESSION"),
        "BOMBCELL": _require_env("BOMBCELL"),
        "NP_FILE_01": _require_env("NP_FILE_01"),
        "NWB_FILE_01": _require_env("NWB_FILE_01"),
        "DATE_01": _require_env("DATE_01"),
        "SESSION_01": _require_env("SESSION_01"),
        "BOMBCELL_01": _require_env("BOMBCELL_01"),
        "NP_FILE_02": _require_env("NP_FILE_02"),
        "NWB_FILE_02": _require_env("NWB_FILE_02"),
        "DATE_02": _require_env("DATE_02"),
        "SESSION_02": _require_env("SESSION_02"),
        "BOMBCELL_02": _require_env("BOMBCELL_02"),
        # Optional keys used for auto-generation of per-session config JSON.
        "RECORDINGS_ROOT": _optional_env("RECORDINGS_ROOT", DEFAULT_RECORDINGS_ROOT) or DEFAULT_RECORDINGS_ROOT,
        "OPEN_EPHYS_CONTINUOUS_SUBPATH": _optional_env(
            "OPEN_EPHYS_CONTINUOUS_SUBPATH",
            DEFAULT_OPEN_EPHYS_CONTINUOUS_SUBPATH,
        ) or DEFAULT_OPEN_EPHYS_CONTINUOUS_SUBPATH,
        "STRUCTURE_OEBIN_SUBPATH": _optional_env(
            "STRUCTURE_OEBIN_SUBPATH",
            DEFAULT_STRUCTURE_OEBIN_SUBPATH,
        ) or DEFAULT_STRUCTURE_OEBIN_SUBPATH,
        "NP20_PROBES": _optional_env("NP20_PROBES", DEFAULT_NP20_PROBES) or DEFAULT_NP20_PROBES,
        "RESULTS_PATH": _optional_env("RESULTS_PATH", "") or "",
        "RUN_DATES": _optional_env("RUN_DATES", "") or "",
    }

    print(f"MOUSE loaded: {data['MOUSE']}")
    print("-- Behavioral Files --")
    print(f"BEHAVIORAL_FOLDER loaded: {data['BEHAVIORAL_FOLDER']}")
    print("-- Neuropixels Sessions --")
    for idx in (1, 2, 3):
        suffix = SESSION_SUFFIX_BY_INDEX[idx]
        print(
            f"Session {idx}: NP_FILE={data[f'NP_FILE{suffix}']} "
            f"DATE={data[f'DATE{suffix}']} SESSION={data[f'SESSION{suffix}']} "
            f"BOMBCELL={data[f'BOMBCELL{suffix}']}"
        )
    print("-- Config Defaults --")
    print(f"RECORDINGS_ROOT loaded: {data['RECORDINGS_ROOT']}")
    print(f"OPEN_EPHYS_CONTINUOUS_SUBPATH loaded: {data['OPEN_EPHYS_CONTINUOUS_SUBPATH']}")
    print(f"STRUCTURE_OEBIN_SUBPATH loaded: {data['STRUCTURE_OEBIN_SUBPATH']}")
    print(f"NP20_PROBES loaded: {data['NP20_PROBES']}")
...
def session_to_analyze(
    MOUSE=None,
    BEHAVIORAL_FOLDER=None,
    NP_FILE=None,
    NWB_FILE=None,
    DATE=None,
    SESSION=None,
    ...
    if session_selection is None:
        raise ValueError("session_selection parameter is not set. Please set it to 1, 2, or 3.")

    if session_selection == 1:
        selected = (NP_FILE, NWB_FILE, DATE, SESSION, BOMBCELL)
    elif session_selection == 2:
        selected = (NP_FILE_01, NWB_FILE_01, DATE_01, SESSION_01, BOMBCELL_01)
    elif session_selection == 3:
        selected = (NP_FILE_02, NWB_FILE_02, DATE_02, SESSION_02, BOMBCELL_02)
    else:
        raise ValueError("session_selection must be 1, 2, or 3.")

    NP_FILE, NWB_FILE, DATE, SESSION, BOMBCELL = selected
    ...
    return MOUSE, BEHAVIORAL_FOLDER, NP_FILE, NWB_FILE, DATE, SESSION, BOMBCELL
