...
from helper_func.grant_config import (
    load_grant_config,
    notebook_runtime_context,
    get_probe_mode_overrides,
)
...
def main() -> None:
    ...
    for probe in probes:
        ...
        try:
            ...
            # Limit joblib worker count used inside BombCell raw waveform extraction.
            # This prevents Windows paging-file exhaustion with n_jobs=-1 on large probes.
            try:
                extract_raw_n_jobs = int(param.get("extractRawNJobs", -1))
            except (TypeError, ValueError):
                extract_raw_n_jobs = 1
            if extract_raw_n_jobs == 0:
                extract_raw_n_jobs = 1
            if extract_raw_n_jobs > 0:
                os.environ["LOKY_MAX_CPU_COUNT"] = str(extract_raw_n_jobs)
